...
<html>
	...
	/*-- Declaration & Initialisation --*/
		...
		/*---Requette SQL permet de verifier l'existance de l'etudiant dans le tab (inscritvous) ---*/
			...
			$res=mysqli_query($con,"SELECT * FROM inscritvous  where  cin ='$cin'") or die ("selct  mdp invalide !!"); 
			...
			if ($rows==0)
				...
				echo  ("<header><h2>pas d'etudiant inscrit avec cette CIN  !!</br><a href='supprimer.html'><b>==> Retour</b></a>  </h2></header>") ; 
			else 
			{
		/*---Requette SQL permet de Supprimer un etudiant dans le tab (inscritvous) ---*/
		
				$res1=mysqli_query($con,"DELETE  FROM inscritvous WHERE  cin='$cin' ") or die ("suppression  echoue  !!"); 

			if ($res1)
			{
				echo  ("<header><h2>Etudiant Supprimer avec succees ^_^  </h2></header>") ; 
				
			}
			else  
			{
				echo  ("<header><h2>  echouee </br><a href='supprimer.php'><b>==> Retour</b></a></h2></header>") ;}
